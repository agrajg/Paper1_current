...
timer on 1
reghdfe ladj_price_per_person ladj_price_per_person100 $ALL_CONTROLS N_cap_inmile_* , /// 
...
*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
reghdfe ladj_price_per_person100 $ALL_CONTROLS ///
	N_cap_inmile_0_500 ///
...
esttab model_set_A* using "05_13_Price_reg_absorbed_propertyid_date_nbhdyw_set_mile_A_final_cap.tex", replace ///
	se /// 
...
esttab model_set_A* using "05_13_Price_reg_absorbed_propertyid_date_nbhdyw_set_mile_A_final_cap.csv", replace ///
	se /// 
	title("\textsc{Impact On Prices}") ///
	b(a2) ///
	order(N_cap_inmile_*) ///
