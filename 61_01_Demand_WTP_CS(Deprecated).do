...
// label var quantity_demanded "Capacity"
// // br if quantity_demanded == 2
// twoway (connected WTP_q1 quantity_demanded, sort) (connected WTP_q3 quantity_demanded, sort) (connected WTP_q4 quantity_demanded, sort), ytitle(Mean Willingness to Pay ($)) ylabel(#8) xlabel(#10) legend(order(1 "Entire Home or Apartment" 2 "Private Room" 3 "Shared Room") position(1) ring(0)) scheme(tufte)
...
//
// twoway (scatter mean_WTP_q2 date, sort) (lfit mean_WTP_q2 date) (scatter mean_WTP_q2 date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(WTP for 2nd unit($)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
...
//
// twoway (scatter mean_CS_exist_actual date, sort) (lfit mean_CS_exist_actual date) (scatter mean_CS_exist_actual date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(Mean Consumer Surplus ($)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
...
//
// twoway (scatter sum_CS_exist_actual date, sort) (lfit sum_CS_exist_actual date) (scatter sum_CS_exist_actual date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(Total Consumer Surplus ($)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
...
drop in 1
texsave v1 v2 v3 v4 v5 using "Y:\agrajg\Research\Paper1_demand_stata\61_01_WTP_table_dta_format" , replace nofix title("\textsc{Willingness To Pay Across Rentals}") size("footnotesize") width("\textwidth") location("h") autonumber hlines(2 5 9 14 42) frag nonames footnote("Notes: " , size("footnotesize"))
...
drop in 1
texsave v1 v2 v3 v4 v5 v6 v8 using "Y:\agrajg\Research\Paper1_demand_stata\61_01_CS_table_dta_format" , replace nofix title("\textsc{Consumer Surplus Across Rentals}") size("footnotesize") width("\textwidth") location("h") autonumber hlines(2 5 9 14 42) frag nonames footnote("Notes: " , size("footnotesize"))
...
twoway (scatter WTP_q2 date, sort) (lfit WTP_q2 date) (scatter WTP_q2 date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(WTP ($)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5)
