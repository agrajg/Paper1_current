...
sum D_d
gen double D_cons = `r(mean)'
// di `D_cons'
* saving beta as a local variable
gen double  D_beta = _b[ladj_price_per_person]
// di `D_beta'
...
capture drop OOS_D_xb_noP
gen double  OOS_D_xb_noP = 	_b[qdemand_l1] * qdemand_l1 + ///
					_b[qdemand_l2] * qdemand_l2 + ///
					_b[p_age] * p_age  + ///
					_b[h_age] * h_age  + /// 
					_b[p_daysbooked] * p_daysbooked  + ///
					_b[h_daysbooked] * h_daysbooked  + /// 
					_b[p_guestcount] * p_guestcount  + /// 
					_b[h_guestcount] * h_guestcount 
...
gen double  OOS_D_xbd_noP = OOS_D_xb_noP + D_FE_propertyid + D_FE_date + D_FE_nbhd_yw + D_FE_nbhd_dow + D_cons
