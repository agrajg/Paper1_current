...
replace sum_quantity = sum_quantity/1000
replace sum_quantity_cf = sum_quantity_cf/1000
twoway (line sum_quantity date, lwidth(vthin)) (line sum_quantity_cf date if date >= td(01nov2015), sort), ytitle("Capacity (1000 persons)") xtitle(Date) xline(20393, lpattern(shortdash_dot)) xlabel(#10, grid) legend(order(1 "Quantity (actual)" 2 "Quantity (counterfactual)") position(1) ring(0)) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
...
drop in 1
texsave v1 v2 v3 v4 v6 v8 v10 using "Y:\agrajg\Research\Paper1_demand_stata\81_01_CS_Actual_CF_dta_format" , replace nofix title("\textsc{Changes in Consumer Welfare}") marker("Table:81_01_CS_Actual_CF_dta_format") size("footnotesize") width("\textwidth") location("t") autonumber hlines(2 5 9 14) frag nonames footnote("Notes: " , size("footnotesize"))
...
drop in 1
texsave v1 v2 v3 v4 v6 v8 v10 using "Y:\agrajg\Research\Paper1_demand_stata\81_01_PS_Actual_CF_dta_format" , replace nofix title("\textsc{Changes in Producer Welfare}") marker("Table:81_01_PS_Actual_CF_dta_format")  size("footnotesize") width("\textwidth") location("t") autonumber hlines(2 5 9 14) frag nonames footnote("Notes: " , size("footnotesize"))
