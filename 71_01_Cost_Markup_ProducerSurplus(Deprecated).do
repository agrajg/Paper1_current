...
twoway (scatter sum_PS_exist_actual date, sort) (lfit sum_PS_exist_actual date) (scatter sum_PS_exist_actual date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(Total Producer Surplus ($)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
...
drop in 1
texsave v1 v6 v7 v8 v9 v2 v3 v4 v5 using "Y:\agrajg\Research\Paper1_demand_stata\71_01_markup_MC_table_dta_format" , replace nofix title("\textsc{Markup and Marginal Cost Across Rentals}") size("footnotesize") width("\textwidth") location("h") autonumber hlines(2 5 9 14 42) frag nonames footnote("Notes: The reported markups are sales wieghted averages." , size("footnotesize"))
...
drop in 1
texsave v1 v2 v3 v4 v5 v6 v7 v8 v9  using "Y:\agrajg\Research\Paper1_demand_stata\71_01_Producer_Profit_Surplus_table_dta_format" , replace nofix title("\textsc{Profit and Producer Surplus Across Rentals}") size("footnotesize") width("\textwidth") location("h") autonumber hlines(2 5 9 14 42) frag nonames footnote("Notes: " , size("footnotesize"))
