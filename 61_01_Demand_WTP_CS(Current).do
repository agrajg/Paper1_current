...
twoway (scatter sum_CS_exist_actual date, sort) (lfit sum_CS_exist_actual date) (scatter sum_CS_exist_actual date if ((week == 52)|(usopendummy==1)), sort mcolor(gray) msize(large) msymbol(circle_hollow)), ytitle(Total Consumer Surplus (1M $)) ylabel(#5, ticks) xtitle(Date) xlabel(#10, grid) legend(off) scheme(tufte) xsize(5) ysize(1.5) scale(1.2)
