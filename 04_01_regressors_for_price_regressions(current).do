...
foreach lname in $DIST_INTERVAL  {
	di `lname'
	gen byte cap_inmile_0_`lname' = (mile >=0 & mile <(`lname'/1000))*purge_capacity
	gen byte cap_inmile_`lname'_plus = (mile >=(`lname'/1000))*purge_capacity 
	gen byte cap_inkm_0_`lname' = (km >=0 & km <(`lname'/1000))*purge_capacity		
	gen byte cap_inkm_`lname'_plus = (km >=(`lname'/1000))*purge_capacity 		
}
global DIST_INTERVAL 500 1000 2000 5000 10000 20000
local prelname = 0
foreach lname in $DIST_INTERVAL  {
	di `prelname'
	di `lname'
	gen byte cap_inmile_`prelname'_`lname' = (mile >=(`prelname'/1000) & mile <(`lname'/1000))*purge_capacity
	gen byte cap_inkm_`prelname'_`lname' = (km >=(`prelname'/1000) & km <(`lname'/1000))*purge_capacity	
	local prelname = `lname'
}

collapse (sum) inmile_* inkm_* cap_inmile_* cap_inkm_* /// 
