...
capture drop pqd
gen double pqd = D_xbd if D_xbd >=0
...
* Marginal Costs
capture drop mc
gen double mc = adj_price_per_person*(1 + (1/elas)) if elas <= -1
...
timer on 2
gen capacity_sq = capacity*capacity	
reghdfe mc  i.cancellationpolicy securitydeposit cleaningfee extrapeoplefee /// 
	...
	i.listingtype i.propertytype i.bedrooms i.bathrooms  capacity capacity_sq if proddum == 1 , ///
...
sum MC_d1
gen double MC_cons1 = `r(mean)'
// di `MC_cons1'

capture drop MC_beta_capacity MC_beta_capacity_sq
gen double MC_beta_capacity = _b[capacity]
gen double MC_beta_capacity_sq = _b[capacity_sq]
...
gen double OOS_MC_xbd1 = MC_xb1 + MC_FE_date1 + MC_FE_nbhd_yw1 + MC_FE_nbhd_dow1 + MC_cons1
*-------------------------------------------------------------------------------
...
gen double OOS_MC_xbd2 = MC_FE_propertyid2 + MC_FE_date2 + MC_FE_nbhd_yw2 + MC_FE_nbhd_dow2 + MC_cons2
*-------------------------------------------------------------------------------
...
esttab model_mc using "10_01_Marginal_Cost_Estimates.tex", replace ///
	...
	b(a2) ///
	order(capacity capacity_sq *listingtype* *bedrooms* *bathrooms*  *propertytype* securitydeposit /// 
