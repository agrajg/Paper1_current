...
reghdfe qdemand lqdemand* p_age h_age p_dayshosting h_dayshosting p_daysbooked h_daysbooked p_guestcount h_guestcount ///
	...
	a(i.propertyid i.nbhd#i.date) cache(save, keep(propertyid date nbhd))

* Regressions
capture estimates clear
*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// timer on 1
// reghdfe qdemand qdemand_* qdemand_l1 p_age h_age p_dayshosting h_dayshosting p_daysbooked h_daysbooked p_guestcount h_guestcount ///
// 	(lprice_per_person = i.prod_week1 i.prod_week2 i.prod_week3 i.prod_week4 i.prod_week5), /// 
// 	a(i.propertyid i.nbhd#i.date) cache(use)
...
esttab model_set_* using "07_04_Demand_lag_reg_absorbed_propertyid_date_nbhdyw.csv", replace ///
	se /// 
